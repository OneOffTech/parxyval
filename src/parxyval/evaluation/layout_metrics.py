...
from parxyval.evaluation.utils import HEADING_LABELS, HEADING_MATCHER_TEXT_SIMILARITY_THRESHOLD, text_similarity


@register_metric("headings_matcher")
def headings_matcher(
        doc: Document,
        gt_doc: Document
) -> dict:
        ...
        for gt in gt_headings:
            for idx, pred in enumerate(pred_headings):
                if idx in matched_preds:
                    continue
                if text_similarity(gt.text.lower(), pred.text.lower()) > HEADING_MATCHER_TEXT_SIMILARITY_THRESHOLD:
                    correct += 1
